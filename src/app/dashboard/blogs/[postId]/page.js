  ...
  if (!post) {
    return (
      <div className="space-y-3">
        <p className="text-sm text-[color:var(--color-light-1)]">Post not found or could not be loaded.</p>
        <Link href="/dashboard/blogs" className="inline-block rounded-xl border border-black/10 bg-white px-4 py-2 text-sm font-medium text-foreground hover:bg-[color:var(--color-light-3)]">
          Back to Blog
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[color:var(--color-light-1)]">Update the post. Use Publish from the list to go live (all images must have alt text).</p>
        </div>
      ...
      <BlogPostForm initialPost={post} submitting={submitting} onSubmit={onSubmit} submitLabel="Update post" />
    </div>
  );
}
